...
/// Emits the IR  computing the total number of iterations in the loop. We
/// don't need to linearize them since we can allocate an nD array instead.
...
emitIterationCounts (mlir::OpBuilder &rewriter, mlir::scf::ParallelOp op)
{
  ...
  for (auto bounds :
       llvm::zip (op.getLowerBound (), op.getUpperBound (), op.getStep ()))
    {
      ...
      Value diff = arith::SubIOp::create (rewriter, op.getLoc (), upperBound,
                                          lowerBound);
      Value count
        = arith::CeilDivSIOp::create (rewriter, op.getLoc (), diff, step);
...
template <typename T>
static mlir::Value
allocateTemporaryBuffer (mlir::OpBuilder &rewriter,
                         mlir::Value value,
