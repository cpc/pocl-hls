//===- SimplifyAffineExprs.h - Ported from Enzyme-JAX ---------------------===//
//
...
extern "C" {
// isl_ast_expr_from_aff is an internal ISL function not in public headers
struct isl_ast_expr;
struct isl_ast_build;
isl_ast_expr *isl_ast_expr_from_aff(__isl_take isl_aff *aff,
...
static LogicalResult addAffineIfOpDomain(AffineIfOp ifOp, bool isElse,
  ...
  // Bail out if the integer set is semi-affine (e.g. symbol*symbol products);
  // FlatAffineValueConstraints asserts on such sets.
  {
    std::vector<SmallVector<int64_t, 8>> flatExprs;
    if (failed(getFlattenedAffineExprs(set, &flatExprs)))
      return failure();
  }
...
struct IslToAffineExprConverter {
  ...
  APInt APIntFromVal(__isl_take isl_val *Val) {
    ...
    APInt A(NumBits, ArrayRef<uint64_t>(Data, NumChunks));
