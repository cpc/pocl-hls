...
public:
  ...
  Match matches(const Offset o) const {
    ...
    llvm_unreachable("unhandled Offset::Type in matches()");
  }
  bool operator<(const Offset o) const {
    ...
    llvm_unreachable("unhandled Offset::Type in operator<()");
  }
};
...
std::unique_ptr<Pass> mlir::pocl::createMem2RegPass() {
...
public:
  ...
  bool definedWithArg(SmallPtrSetImpl<Block *> &block) {
    ...
    llvm_unreachable("unhandled case in definedWithArg()");
  }
  Value materialize(bool full = true) {
    ...
    llvm_unreachable("unhandled case in materialize()");
  }
  ...
  for (auto &pair : blocksWithAddedArgs) {
    ...
    for (auto *pred : prepred) {
      ...
      if (auto op = dyn_cast<cf::BranchOp>(pred->getTerminator())) {
        ...
        auto newop = cf::SwitchOp::create(builder, op.getLoc(), op.getFlag(),
      ...
      while (list.size()) {
        ...
        for (auto *U : val.getUsers()) {
          if (auto SO = dyn_cast<memref::StoreOp>(U)) {
            ...
            toErase.push_back(U);
            list.push_back(CO);
          } else if (auto CO = dyn_cast<pocl::SubIndexOp>(U)) {
            toErase.push_back(U);
            list.push_back(CO);
          } else {
            error = true;
