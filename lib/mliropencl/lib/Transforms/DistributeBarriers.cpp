...
#include "pocl/Utils/BarrierUtils.h"
#include "pocl/Utils/Utils.h"

#include <deque>

namespace {
#define GEN_PASS_DEF_DISTRIBUTEBARRIERS
#include "pocl/Transforms/Passes.h.inc"
} // namespace

#define DEBUG_TYPE "distribute-barriers"
...
static void minCutCache(pocl::BarrierOp barrier,
...
static void findValuesUsedBelow(pocl::BarrierOp op,
    ...
    for (Value value : current.first->getResults()) {
      for (Operation *user : value.getUsers()) {
        ...
        if (!op->isBeforeInBlock(user)) {
          ...
          if (!isa<memref::LoadOp, AffineLoadOp>(origUser)) {
  ...
  for (auto v : crossing) {
    if (isa<memref::AllocaOp>(v.getDefiningOp())) {
...
static bool hasNestedBarrier(Operation *op, SmallVector<BlockArgument> &vals) {
  op->walk([&](pocl::BarrierOp barrier) {
...
LogicalResult splitSubLoop(AffineParallelOp op, PatternRewriter &rewriter,
  ...
  if (!innerLower.size())
    return failure();
  if (outerLower.size()) {
    outerLoop = AffineParallelOp::create(
  ...
  for (auto tup : llvm::zip(innerLower, innerUpper, innerStep)) {
    ...
    ops.append(symbols);
    iterCounts.push_back(AffineApplyOp::create(
        rewriter, op.getLoc(),
        AffineMap::get(dims.size(), symbols.size(), expr), ops));
  }
  preLoop = AffineParallelOp::create(rewriter, op.getLoc(), TypeRange(),
                                     ArrayRef<AtomicRMWKind>(), innerLower,
                                     op.getLowerBoundsOperands(), innerUpper,
                                     op.getUpperBoundsOperands(), innerStep);
  rewriter.eraseOp(&preLoop.getBody()->back());
  postLoop = AffineParallelOp::create(rewriter, op.getLoc(), TypeRange(),
                                      ArrayRef<AtomicRMWKind>(), innerLower,
                                      op.getLowerBoundsOperands(), innerUpper,
                                      op.getUpperBoundsOperands(), innerStep);
  rewriter.eraseOp(&postLoop.getBody()->back());
...
static LogicalResult distributeAroundBarrier(T op, pocl::BarrierOp barrier,
  ...
  auto addToAllocations = [&](Value v, SmallVector<Value> &allocations) {
    if (auto cl = v.getDefiningOp<pocl::CacheLoad>()) {
      ...
      for (auto &u : llvm::make_early_inc_range(ao.getResult().getUses())) {
        ...
        for (auto idx : preLoop.getBody()->getArguments()) {
          ...
          auto mt = MemRefType::get(shape, mt0.getElementType(),
                                    ...
                                    mt0.getMemorySpace());
          auto subidx =
              pocl::SubIndexOp::create(rewriter, alloc.getLoc(), mt, buf, idx);
          buf = subidx;
        }
        u.set(buf);
      }
  ...
  for (auto pair : llvm::zip(crossingCache, cacheAllocations)) {
    ...
    if (!isa<pocl::CacheLoad>(v.getDefiningOp())) {
      // Store
    ...
    // Reload
    rewriter.setInsertionPointAfter(barrier);
    Value reloaded = pocl::CacheLoad::create(rewriter, v.getLoc(), alloc,
                                             preLoop.getBody()->getArguments());
    for (auto &u : llvm::make_early_inc_range(v.getUses())) {
      auto *user = u.getOwner();
      while (user->getBlock() != barrier->getBlock())
  ...
  if (outerLoop) {
    if (isa<scf::ParallelOp>(outerLoop))
      ...
      AffineYieldOp::create(rewriter, op.getLoc());
    }
  } else {
...
static pocl::BarrierOp getFirstBarrier(Block *block) {
  auto it = llvm::find_if(block->getOperations(), [](Operation &nested) {
    return isa<pocl::BarrierOp>(nested);
  });
  if (it == block->end()) {
    return nullptr;
  }
  return cast<pocl::BarrierOp>(&*it);
}

template <typename T, bool UseMinCut>
static LogicalResult distributeAroundFirstBarrier(T op, T &preLoop, T &postLoop,
                                                  PatternRewriter &rewriter) {
  pocl::BarrierOp barrier = getFirstBarrier(op.getBody());
  if (!barrier)
    return failure();
...
bool isBarrierContainingAll(Operation *op, SmallVector<BlockArgument> &args) {
  auto bar = dyn_cast<pocl::BarrierOp>(op);
  if (!bar)
    return false;
...
wrapWithBarriers(T op, PatternRewriter &rewriter,
                 ...
                 pocl::BarrierOp &before, pocl::BarrierOp &after) {
  ...
  if (!hasPrevBarrierLike)
    before = pocl::BarrierOp::create(rewriter, op->getLoc(), vargs);

  if (!hasNextBarrierLike) {
    OpBuilder::InsertionGuard guard(rewriter);
    rewriter.setInsertionPointAfter(op);
    after = pocl::BarrierOp::create(rewriter, op->getLoc(), vargs);
  }
...
static LogicalResult wrapWithBarriers(T op, PatternRewriter &rewriter,
                                      SmallVector<BlockArgument> &args,
                                      bool recomputable) {
  pocl::BarrierOp before, after;
  return wrapWithBarriers(op, rewriter, args, recomputable, before, after);
}

template <typename T, bool UseMinCut>
static LogicalResult
distributeAfterWrap(Operation *pop, pocl::BarrierOp barrier,
...
SmallVector<Value> getLowerBounds(AffineParallelOp op,
  ...
  for (AffineExpr expr : op.getLowerBoundsMap().getResults()) {
    vals.push_back(AffineApplyOp::create(rewriter, op.getLoc(), expr,
...
static pocl::BarrierOp getBarrierInBlock(Block *block) {
  for (Operation &op : *block) {
    if (auto barrier = dyn_cast<pocl::BarrierOp>(&op))
      return barrier;
  }
  return nullptr;
}
...
struct DistributeIfAroundBarrier : public OpRewritePattern<IfOpType> {
  ...
  LogicalResult matchAndRewrite(IfOpType ifOp,
    ...
    pocl::BarrierOp thenBarrier = nullptr;
    pocl::BarrierOp elseBarrier = nullptr;
    if (thenBlock)
    ...
    if (!thenBarrier) {
      ...
      thenBarrier = pocl::BarrierOp::create(rewriter, ifOp->getLoc(),
                                            elseBarrier->getOperands());
    }
    if (elseBlock) {
      if (!elseBarrier) {
        rewriter.setInsertionPoint(getElseBlock(ifOp)->getTerminator());
        elseBarrier = pocl::BarrierOp::create(rewriter, ifOp->getLoc(),
    ...
    auto handleCase = [&ifPre, &rewriter, &ifOp](pocl::BarrierOp barrier,
      ...
      for (auto pair : llvm::zip(crossingCache, cacheAllocations)) {
        ...
        rewriter.setInsertionPointAfter(barrier);
        Value reloaded =
            pocl::CacheLoad::create(rewriter, v.getLoc(), alloc, ValueRange());
        for (auto &u : llvm::make_early_inc_range(v.getUses())) {
          auto *user = u.getOwner();
          while (user->getBlock() != barrier->getBlock())
            user = user->getBlock()->getParentOp();
...
struct Reg2MemFor : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T op,
    ...
    if (UseMinCut)
      loadValues<pocl::CacheLoad>(op.getLoc(), allocated, rewriter,
                                  newRegionArguments);
    else
    ...
    while (IP != &IP->getBlock()->front()) {
      if (isa<pocl::BarrierOp>(IP->getPrevNode())) {
        IP = IP->getPrevNode();
      }
      break;
    }
    ...
    for (Value alloc : allocated) {
      if (UseMinCut)
        loaded.push_back(
            pocl::CacheLoad::create(rewriter, op.getLoc(), alloc, ValueRange())
                ->getResult(0));
      else
        loaded.push_back(
...
bool isEquivalent(Value a, Value b) {
  ...
  if (a.getType() != b.getType())
    return false;
  if (auto sa = a.getDefiningOp<pocl::SubIndexOp>()) {
    if (auto sb = b.getDefiningOp<pocl::SubIndexOp>()) {
      ...
      for (auto user : res.getUsers()) {
        if (auto storeOp = dyn_cast<memref::StoreOp>(user)) {
          ...
          if (!usesMustStore) {
            ...
            if (auto cl = val.getDefiningOp<pocl::CacheLoad>()) {
      ...
      if (alloc) {
        ...
        if (UseMinCut)
          std::get<0>(pair).replaceAllUsesWith(
              pocl::CacheLoad::create(rewriter, op.getLoc(), alloc,
                                      ValueRange())
                  ->getResult(0));
        else
          std::get<0>(pair).replaceAllUsesWith(
...
struct LowerCacheLoad : public OpRewritePattern<pocl::CacheLoad> {
  using OpRewritePattern<pocl::CacheLoad>::OpRewritePattern;

  LogicalResult matchAndRewrite(pocl::CacheLoad op,
                                PatternRewriter &rewriter) const override {
...
struct DistributeBarriers
    : public impl::DistributeBarriersBase<DistributeBarriers> {
  template <bool UseMinCut>
  ...
  DistributeBarriers() = default;
  DistributeBarriers(DistributeBarriersOptions method) {
    this->method.setValue(method.method);
  }
...
std::unique_ptr<Pass>
mlir::pocl::createDistributeBarriersPass(DistributeBarriersOptions str) {
  return std::make_unique<DistributeBarriers>(str);
}
std::unique_ptr<Pass> mlir::pocl::createDistributeBarriersPass() {
  DistributeBarriersOptions str = {"distribute"};
  return std::make_unique<DistributeBarriers>(str);
}
