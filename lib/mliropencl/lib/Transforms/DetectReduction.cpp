  ...
  LogicalResult matchAndRewrite(LoopLikeOpInterface Loop,
    ...
    // a store right after the for. The stored element is the result of the for.
    {
      ...
      for (ReductionOp &Op : ReductionOps) {
        ...
        auto Store = affine::AffineStoreOp::create(
            Rewriter, NewLoop.getLoc(),
...
public:
  ...
  createNewLoop(LoopLikeOpInterface Loop, SmallVectorImpl<Value> &NewIterArgs,
    ...
    return scf::ForOp::create(Rewriter, ForOp.getLoc(), ForOp.getLowerBound(),
...
public:
  ...
  createNewLoop(LoopLikeOpInterface Loop, SmallVectorImpl<Value> &NewIterArgs,
    ...
    return affine::AffineForOp::create(
        ...
        ForOp.getUpperBoundMap(), ForOp.getStepAsInt(), NewIterArgs);
  }
};
...
void DetectReductionPass::runOnOperation() {
  ...
  (void)applyOpPatternsGreedily(LoopOps, std::move(RPS), Config);
}
