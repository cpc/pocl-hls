// RUN: pocl-mlir-opt --pocl-distribute-barriers="method=distribute" --allow-unregistered-dialect --canonicalize --split-input-file %s | %FileCheck %s
...
module {
  ...
  func.func @main() {
    ...
    scf.parallel (%arg2) = (%c0) to (%c5) step (%c1) {
	  ...
	  memref.store %c0_i8, %0[] : memref<i8>
          "pocl.barrier"(%arg3) : (index) -> ()
          scf.yield %c0_i8 : i8
        }
    ...
    %c1 = arith.constant 1 : index
        affine.parallel (%arg15, %arg16) = (0, 0) to (16, 16) {
            scf.for %arg17 = %c0 to %len step %c1 {
              ...
              "pocl.barrier"(%arg15, %arg16, %c0) : (index, index, index) -> ()
            }
        }
    return
  }
...
// CHECK-NEXT:      %[[V0:.+]] = memref.alloca() : memref<i8>
...
// CHECK-NEXT:       } do {
// CHECK-NEXT:         scf.parallel (%[[arg1:.+]]) = (%[[c0]]) to (%[[c2]]) step (%[[c1]]) {
// CHECK-NEXT:           memref.store %[[c0_i8]], %[[V0]][] : memref<i8>
...
// CHECK-NEXT:       }
// CHECK-NEXT:       }
// CHECK-NEXT:       "test.use"(%[[V0]]) : (memref<i8>) -> ()
// CHECK-NEXT:       scf.reduce
// CHECK-NEXT:     }
// CHECK-NEXT:     return
// CHECK-NEXT:   }
